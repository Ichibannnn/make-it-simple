...
//     if (editData) {
//       // USER LENGTH
//       const userIdsLength = formData.userId?.map((item) => item.userId).length || 0;
...
//         payload.append(`AddRequestConcernbyConcerns[${i}].concern_Details`, concernDetails[i % concernDetails.length]);
//       }
//       // Category
//       const category = [formData.categoryId?.id];
//       for (let i = 0; i < userIdsLength; i++) {
//         payload.append(`AddRequestConcernbyConcerns[${i}].categoryId`, category[i % category.length]);
//       }
//       // SubCategory
...
//       for (let i = 0; i < userIdsLength; i++) {
//         payload.append(`AddRequestConcernbyConcerns[${i}].start_Date`, startdate[i % startdate.length]);
//       }
//       // TargetDate
//       const targetdate = [moment(formData.targetDate).format("YYYY-MM-DD")];
//       for (let i = 0; i < userIdsLength; i++) {
//         payload.append(`AddRequestConcernbyConcerns[${i}].target_Date`, targetdate[i % targetdate.length]);
//       }
...
//       const subcategory = [formData.subCategoryId?.id];
//       for (let i = 0; i < userIdsLength; i++) {
//         payload.append(`AddRequestConcernbyConcerns[${i}].subCategoryId`, subcategory[i % subcategory.length]);
//       }
//       // IssueHandler
//       const assignto = formData.userId?.map((item) => item.userId);
//       for (let i = 0; i < assignto.length; i++) {
...
//       const ticketConcernIdArray = editData?.ticketRequestConcerns?.map((item) =>
//         item?.ticketConcerns?.map((item2) => {
//           return {
//             ticketConcernId: item2?.ticketConcernId,
//           };
...
//         category_Description: bindData?.[0]?.category_Description ? bindData?.[0]?.category_Description : "",
//       });

//       setValue("subCategoryId", {
//         id: bindData?.[0]?.subCategoryId ? bindData?.[0]?.subCategoryId : "",
//         subCategory_Description: bindData?.[0]?.subCategory_Description ? bindData?.[0]?.subCategory_Description : "",
//       });

//       setValue("ChannelId", {
//         id: bindData?.[0]?.channelId ? bindData?.[0]?.channelId : "",
//         channel_Name: bindData?.[0]?.channel_Name ? bindData?.[0]?.channel_Name : "",
//       });
...
//                           borderTopRightRadius: "20px",
//                           backgroundColor:
//                             addData?.requestTransactionId === item?.requestTransactionId
//                               ? "#5f478e"
//                               : editData?.requestTransactionId === item?.requestTransactionId
//                               ? "#5f478e"
//                               : "",
//                           "&:hover": {
//                             backgroundColor: theme.palette.secondary.main,
...
//               <form onSubmit={handleSubmit(onSubmitAction)}>
//                 <Stack sx={{ minHeight: "70px" }}>
//                   <Typography sx={{ fontWeight: 600, fontSize: "18px" }}>Manage Ticket Details</Typography>
//                   <Typography
//                     sx={{
//                       fontSize: "14px",
//                       color: theme.palette.text.secondary,
...
//                                   textField: { variant: "outlined" },
//                                 }}
//                                 minDate={startDateValidation || minEditStartDate} // onChange Start Date
//                                 error={!!errors.targetDate}
//                                 helperText={errors.targetDate ? errors.targetDate.message : null}
//                                 disabled={!watch("startDate")}
//                               />
